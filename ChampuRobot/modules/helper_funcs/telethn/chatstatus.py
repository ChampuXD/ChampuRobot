...
from ChampuRobot import DRAGONS
from ChampuRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Champu_is_admin(chat_id: int):
    status = False
    Champu = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Champu.id == user.id:
            status = True
            break
    return status
