...
from ChampuRobot import BOT_ID
from ChampuRobot import DRAGONS as DEVS
from ChampuRobot import OWNER_ID
from ChampuRobot import telethn as Champu
from ChampuRobot.events import Champuinline
from ChampuRobot.events import register as Champubot
from ChampuRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Champu(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Champu(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Champubot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
*ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ:*
   ...
   •➥ *ꜱᴇᴛᴜᴘ*
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/TheChampuClub?startgroup=new)
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ your ᴄʜᴀɴɴᴇʟ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/TheChampu?startgroup=new)
